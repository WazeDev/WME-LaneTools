...
`;
    ...
    let _turnData = {};
    let laneCount = 0;
    const LTHighlightLayer: LayerDescriptor = { name: "LT Highlights Layer" };
    ...
    async function setupOptions() {
        ...
        $(".lt-checkbox").on("click", function () {
            ...
            LtSettings[settingName] = (this as HTMLInputElement).checked;
            saveSettings();
        });
    ...
    async function loadSpreadsheet() {
        ...
        const rbsFailFunc = (_jqXHR: null | undefined, _textStatus: null | undefined, errorThrown: unknown) => {
        ...
        const translationsFailFunc = (jqXHR: null, textStatus: null, errorThrown: string) => {
    ...
    function lanesTabSetup() {
            ...
            if (lanes.find(".lane-instruction-to").children().length > 0x0 && !getId(addLanesTag)) {
                ...
                waitForElementLoaded(prependSelector).then((elm) => {
                    ...
                    $(".lt-add-lanes").on("click", function () {
                        ...
                        const numAdd: number = Number.parseInt(numAddStr, 10);
        ...
        function colorCSDir() {
            ...
            const fwdConfig: LaneConfiguration = checkLanesConfiguration(
                selSeg,
                fwdNode,
                fwdNode ? fwdNode.connectedSegmentIds : [],
                selSeg?.toNodeLanesCount
            );
            const revConfig: LaneConfiguration = checkLanesConfiguration(
                selSeg,
                revNode,
    ...
    function applyName(geo: GeoJSON.Position, fwdLnsCount: number, revLnsCount: number) {
        // if (!fwdLnsCount) fwdLnsCount = 0;
        // if (!revLnsCount) revLnsCount = 0;
        const laneNum = `${fwdLnsCount} / ${revLnsCount}`;
        const lnLabel: GeoJSON.Feature = turf.point(
            geo,
    ...
    function highlightSegment(
        ...
        if (objGeo.length > 2) {
            ...
            if (direction === Direction.FORWARD) {
                ...
                pVector.push(p2C);
                const newString: GeoJSON.Feature = turf.lineString(
                    pVector,
    ...
    function scanHeuristicsCandidates(selection: Selection) {
        ...
        for (let idx = 0; selection && idx < selection.ids.length; ++idx) {
            ...
            const seg: Segment | null = sdk.DataModel.Segments.getById({ segmentId: selection.ids[idx] });
            if (!seg) continue;
            count = segs.push(seg);
        }
        // _.each(features, (f) => {
    ...
    function scanSegments(segments: Segment[], selectedSegsOverride: boolean = false) {
        ...
        function scanSegment_Inner(seg: Segment, direction: number, segLength: number, tryRedo: boolean) {
            ...
            const entrySegRef: SegmentReference = {
                seg: 0,
            ...
            if (node !== null && onScreen(node, zoomLevel)) {
                ...
                if (laneCount && laneCount > 0) {
                    const config: LaneConfiguration = checkLanesConfiguration(seg, node, nodeSegs, laneCount);
                    tlns = config.tlns;
                    tio = config.tio;
                    lio = config.lio;
                    badLn = config.badLn;
    ...
    function checkLanesConfiguration(
        ...
        const laneConfig: LaneConfiguration = {
            tlns: false,
            tio: false,
            badLn: false,
            lio: false,
        ...
        const pturns: Turn[] = fromTurns.filter((t: Turn) => t.isPathTurn);
        pturns.push(...toTurns.filter((t) => t.isPathTurn));

        const jpturns: Turn[] = fromTurns.filter((t: Turn) => t.isJunctionBoxTurn);
            ...
            for (let idx = 0; idx < turnsThrough.length; ++idx) {
                ...
                if (t.isAllowed) {
                    ...
                    if (t.lanes !== null) {
                        ...
                        const primaryStreetId: number | null | undefined = seg2?.primaryStreetId;
    ...
    function isHeuristicsCandidate(
        ...
        for (let ii = 0; ii < nodeEntrySegIds.length; ii++) {
            ...
            if (!inSegRef) {
                const newSegRef: SegmentReference = {
                    seg: 0,
                    direction: Direction.ANY,
                };
                inSegRef = newSegRef;
        ...
        function lt_is_turn_allowed(s_from: Segment, via_node: Node, s_to: Segment | null) {
            const turnsThrough: Turn[] = sdk.DataModel.Turns.getTurnsThroughNode({ nodeId: via_node.id });
            ...
            const permissions: TurnPermissions = isTurnAllowedBySegDirections(s_from, s_to);
            lt_log(
    ...
    function lt_log(lt_log_msg: string, verbosity_level: VERBOSITY = VERBOSITY.INFO) {
        // ##NO_FF_START##
    ...
    function copyLaneInfo(side: string) {
        _turnInfo = [];
        ...
        for (let i = 0; i < conSegs.length; i++) {
            const seg2: Segment | null = getSegObj(conSegs[i]);
    ...
    function pasteLaneInfo(side: string) {
        const mAction = new MultiAction();
        //    mAction.setModel(W.model);
    ...
    function drawIcons(seg: Segment | null, node: Node | null, imgs: any, isLeftDrive: boolean = false) {
        ...
        const boxRing = turf.polygon(
            turfBoxRing.geometry.coordinates,
            { styleName: "boxStyle", layerName: LTLaneGraphics.name },
        ...
        _.each(imgs, (img) => {
            ...
            const iconBoxRing = turf.polygon(
                turfIconBoxRing.geometry.coordinates,
                { styleName: "iconBoxStyle", layerName: LTLaneGraphics.name },
            ...
            let ulabel = "";
            const usize: Coordinates = {
                x: undefined,
                y: undefined,
            };
            const uoffset: Coordinates = {
                x: undefined,
                y: undefined,
            };
            if (img.uturn === true) {
            ...
            const iconStart: GeoJSON.Feature = turf.point(
